/**
 * Where the two protocols meet: one A2A task, translated into one ACP prompt turn.
 *
 * Upstream is A2A — a stateful task, streamed status updates, an artifact at the end.
 * Downstream is ACP — an adapter subprocess that streams message chunks, tool calls and
 * plans. The switch in `turn()` is the whole point of the exercise, and `settle()` is
 * where a `stopReason` becomes a `TaskState`.
 *
 * It is kept out of `src/acp/agent.ts` because that file is a bootstrap: importing it
 * reads `.env`, spawns an adapter for the handshake and binds a port. The translation
 * itself needs none of that — it needs a runtime it can prompt, which is what
 * `RuntimeSource` asks for and what `AcpRegistry` happens to be.
 */
...
import { RequestError, type ActiveSession, type StopReason, type Usage } from '@agentclientprotocol/sdk';
import { agentMessage, dataPart, readText, statusUpdate, textPart } from '../parts.ts';
import type { Backend, Budget } from './client.ts';
import type { Logs } from './log.ts';
import type { Boundary } from './permissions.ts';

/**
 * What one conversation has to offer for a turn to run in it.
 *
 * Named as an interface rather than taken as the `Runtime` class so the translation can
 * be exercised without a subprocess behind it. `AcpRegistry`'s runtimes satisfy it as
 * they are — this is a description of the existing shape, not a wrapper around it.
 */
export interface TurnRuntime {
  readonly openTasks: Set<string>;
  readonly sessionId: string;
  readonly boundary: Boundary;
  readonly session: Pick<ActiveSession, 'prompt' | 'nextUpdate'>;
...
/** The half of `AcpRegistry` this executor uses. */
export interface RuntimeSource {
  acquire(contextId: string): Promise<TurnRuntime>;
}
...
export const describeError = (err: unknown, backend: Backend): string => {
...
export class AcpExecutor implements AgentExecutor {
  private readonly runtimeByTask = new Map<string, TurnRuntime>();
  ...
  cancelTask = async (taskId: string): Promise<void> => {
    ...
    try {
      ...
      console.log(`  ↯ cancel ${taskId.slice(0, 8)}: notification failed — ${describeError(err, this.backend)}`);
    }
  };
  ...
  async execute(ctx: RequestContext, bus: ExecutionEventBus): Promise<void> {
    ...
    try {
          ...
          AgentEvent.statusUpdate(
            statusUpdate(
              ...
              agentMessage(taskId, contextId, [
                textPart(`Tell me what to do — reply into this same task and ${this.backend.label} picks it up.`),
              ]),
            ),
          ),
        );
      ...
      terminal = true;
      const reason = describeError(err, this.backend);
  ...
  private settle(
    ...
    if (stopReason === 'refusal') {
      console.log('  ⤷ refused');
      return done(TaskState.TASK_STATE_FAILED, answer || `${this.backend.label} refused the request.`);
    }
    ...
    const artifact: Artifact = {
      ...
      name: 'acp-answer',
      description: `Answer produced by ${this.backend.label} over ACP`,
      parts: [
